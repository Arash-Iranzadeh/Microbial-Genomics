...
TOTAL_TB="${TOTAL_TB:-20}"             # how many TB runs to select
...
CURL_HEAD_OPTS=( -fsSI --retry 4 --retry-delay 2 --connect-timeout 20 )
if [[ "$FORCE_IPV4" == "true" ]]; then CURL_GET_OPTS=( -4 "${CURL_GET_OPTS[@]}" ); CURL_HEAD_OPTS=( -4 "${CURL_HEAD_OPTS[@]}" ); fi
...
main(){
  log "[TB] Preparing TB into ${TB_DIR}"
  ...
  log "[TB] Querying ENA ..."
  ena_search "$q" "$all"; validate_or_die "$all"

  log "[TB] Selecting ${TOTAL_TB} runs ..."
  select_unique_country_then_fill "$all" "$TOTAL_TB" "$pick"
  append_class_and_header "$pick" "clinical_or_unknown" "$runs"
  write_metadata "$runs" "$meta"
  add_reported_amr_from_sample_attributes "$runs" "$TB_DIR/tb_metadata_plus_amr.tsv"

  if [[ "$DOWNLOAD_FASTQS" == "true" ]]; then
    log "[TB] Downloading FASTQs (no size cap) ..."
    download_fastqs_simple "$runs" "$TB_DIR"
  fi
